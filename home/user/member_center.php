...
<form action="action.php?act=<?php echo $action?>" method="post" enctype="multipart/form-data">
	<table width="50%"  border="1" cellpadding="5" cellspacing="0" bgcolor="#cccccc" align="center">
		<!--		<caption><h2>--><?php //echo $title?><!--</h2></caption>-->
		<caption><h2>个人中心</h2></caption>
		<tr>
		...
		<tr>
			...
			<td><input type="file" name="pic">
				...
				if(isset($row)){
					//处理头像
					$path = '../../public/upload/thumbHead/s_80_80_'.$row['pic'];
					if(!file_exists($path)){
						//如果略缩图文件不存在,执行缩放函数
						$thumb = thumb('../../public/upload/head/'.$row['pic'],80,80,'../../public/upload/thumbHead');
						$path = $thumb['pathInfo'];
					}
					//echo $path;
					echo '<img src="'.$path.'">';

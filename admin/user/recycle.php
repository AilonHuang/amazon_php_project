...
<div class="details">
	...
	<table class="table" cellspacing="0" cellpadding="0">
		...
		<tbody>
		<form action="action.php?act=selected" method="post">
			<?php
			...
			$search = search();
			//调用分页函数
			$page= page(5,$link,$search['sql'],1);
			$url = "&keywords={$search['keywords']}&level={$search['level']}&id1={$search['id1']}&id2={$search['id2']}";
			//准备sql语句
			$sql = "SELECT * FROM users WHERE isdel=1 {$search['sql']}  ORDER BY id ASC {$page['limit']}";
			//echo $sql;//die;
			//发送sql语句
			...
			<tr>
			    ...
			    <td>{$time}</td>
			    <td align="center">
				    <a href="action.php?act=recover&id={$row['id']}"><input type="button" value="恢复" class="btn" ></a>
				    <a href="action.php?act=realDel&id={$row['id']}"><input type="button" value="彻底删除" class="btn" ></a>
...
EOF;
			...
			$str = <<<EOF
			 <tr>		
			     <td ><input type="submit" name='realDelSelected' value="批量删除" class="btn" ><input type="submit" name="recoverSelected" value="批量还原" class="btn" ></td>
                <td colspan="5" align="right" valign="top">{$page['num']}条数据 {$page['dPage']}/{$page['all']}页&nbsp;&nbsp;
                    ...
                    <a href="?page={$page['all']}$url" target="mainFrame" >尾页</a></td>
            </tr>
EOF;
			echo $str;
			?>
